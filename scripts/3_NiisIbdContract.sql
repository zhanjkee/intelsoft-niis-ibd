...
SELECT ROW_NUMBER() OVER(ORDER BY [P].[U_ID] ASC) AS [Id]
	  ...
	  ,ISNULL([Owner].[Xin], '') AS [Xin]
	  ,ISNULL([Owner].[PatentHolderRu], '') AS [Name]
	  ,[R].[flChildDocId] AS [PropertyId]
	  ,ISNULL([dbo].[FUNC_PATENT_STORONA2_XIN]([P].[U_ID]), '') AS [AssigneeXin]
	  ,ISNULL([dbo].[FUNC_PATENT_STORONA2]([P].[U_ID]), '') AS [AssigneeName]
